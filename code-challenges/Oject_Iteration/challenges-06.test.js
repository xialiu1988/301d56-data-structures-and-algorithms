...
const houseSize = (arr) => {
  ...
  for (let i in arr){
    ...
    if(arr[i].spouse!==null){
      ...
      obj.members=2+arr[i].children.length;
      sizes.push(obj);

    }
    else{
      obj.house=arr[i].house;
      obj.members=1+arr[i].children.length;
      sizes.push(obj);
    }
...
const houseSurvivors = (arr) => {
  ...
  for(let i in arr){
    let obj={};
    if (arr[i].spouse===deceasedSpouses[0]||arr[i].spouse===deceasedSpouses[1]||arr[i].spouse===deceasedSpouses[2]||arr[i].spouse===deceasedSpouses[3]||arr[i].spouse===deceasedSpouses[4]||arr[i].spouse===null){
      obj.house=arr[i].house;
      obj.members=1+arr[i].children.length;
      survivors.push(obj);
    }
