...
const countNumberOfElements = (arr) => {
  ...
  let num=arr.reduce(function(acc,value,idx){
    acc++;
    return acc;
  },0);
  return num;
...
const reversedString = (str) => {
  // Solution code here...
  let arr=str.split('');
  let newarr=arr.reduce(function(fun,value,idx){

    return value.concat(fun);
  },[]);
  return newarr;
};
...
const extractStat = (statName, arr) => {
  ...
  let newarr=arr.reduce(function(acc,item,idx){
    if(item.stat.name===statName){
      acc.stat=item.stat;
      acc.effort=item.effort;
      acc.baseStat=item.baseStat;
    }
    return acc;
  },{});
  return newarr;
